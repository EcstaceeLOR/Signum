...
assert.ok(
  javascriptGzipBytes <= 115 * 1024,
  `JavaScript gzip budget exceeded: ${formatKilobytes(javascriptGzipBytes)} > 115 kB.`,
)
assert.ok(
...
assert.ok(
  totalGzipBytes <= 123 * 1024,
  `Combined gzip budget exceeded: ${formatKilobytes(totalGzipBytes)} > 123 kB.`,
)
